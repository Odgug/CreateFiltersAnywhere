...
public class Config {
    ...
    static {
        BUILDER.push("CreateFiltersAnywhere Server Config");
        ...
        TOMS_COMPAT = BUILDER.comment("\nEnable compatibility for list and attribute filters inside Tom's filter item" +
                "\nDefault:true").define("Tom's Simple Storage Compatibility", true);

        LASERIO_COMPAT = BUILDER.comment("\nEnable compatibility for list and attribute filters inside Laser IO's basic filter card" +
